...
# Your Cox model
cox_model <- coxph(Surv(time, event) ~ log(creatinine+1)+age + ef_cat + bp +
                       anaemia + sodium, data = data)
...
boot_c_statistic <- function(original_data, indices) {
  ...
  fit <- coxph(Surv(time, event) ~ log(creatinine+1) + age + ef_cat + bp + anaemia + sodium, data = boot_data)
...
boot_results_c_stat <- boot(data = data, statistic = boot_c_statistic, R = 400)
...
# Perform bootstrap
set.seed(123)
boot_results_logistic <- boot(data = data, statistic = boot_calibration_logistic, R = 400)
...
# Perform bootstrap
set.seed(123)
boot_results_logistic <- boot(data = data, statistic = boot_calibration_logistic, R = 200)
