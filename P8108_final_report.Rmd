...
$$
\hat{S}_L(t_i)=\hat{S}_L(t_{i-1})(1-\frac{d_{i}}{{n_{i}'}})
$$
...
1 & \text { if } t< t_1  \\ 
\prod_{t_i \le t} \exp[-\frac{d_i}{n_i}] = \prod_{t_i \le t} \exp[-h_i] & \text { if }   t \ge t_1
\end{cases}
$$
...
The Log-Rank test focuses on comparing the number of observed to expected events across the groups at each time point. The test statistic^[This is the test statistics with tie] is calculated as:

$$
\chi^2_{\text{LR}} = \frac{\sum_{i=1}^{k}(d_{0i} - e_{0i})}{\sum_{i=1}^{k}\sqrt{\frac{n_{0i}n_{1i}d_i(n_i-d_i)}{n_i^2(n_i-1)}}}
$$

The Gehan's Wilcoxon test^[Also known as the Breslow test] gives more weight to events at earlier time points. The test statistic is calculated as:

$$
\chi^2_{\text{GW}} = \frac{\sum_{i=1}^{k}n_i(d_{0i} - e_{0i})}{\sum_{i=1}^{k}\sqrt{\frac{n_{0i}n_{1i}d_i(n_i-d_i)}{n_i-1}}}
$$
Obviously, the Gehan's Wilcoxon test is actually a special case of weighted log-rank test with weight equals $n_i$.
...
\item $\gamma = \text{Shape parameter}$ 
...
\item $\eta = \text{Rate of increase or decrease in hazard}$ 
