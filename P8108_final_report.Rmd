...
where $O_{ij}$ is the observed number of events (e.g., deaths) in group  j at time i, $E_{ij}$ is the expected number of events in group  $j$ at time  $i$,  $n_{ij}$ is the number of individuals at risk in group j at time $i$, $O_i$ is the total number of events at time $i$, and $n_i$ is the total number at risk at time $i$.

The Wilcoxon test achieves a greater sensitivity to differences in survival that manifest at the beginning of the observation period. The test statistic is calculated as:
$$
\chi^2_{\text{Wilcoxon}} = \sum_{i=1}^{k} \frac{(O_{i1} - E_{i1})^2}{V_{i1}} + \frac{(O_{i2} - E_{i2})^2}{V_{i2}}
$$

$V_{ij}$ represents the variance of the observed number of events in group $j$ at time $i$. The weight factor by $n_{ij}$ in $V_{ij}$ plays a crucial role. It ensures that the survival differences at earlier time points carry more weight in the overall analysis. 
