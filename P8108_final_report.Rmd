...
Our project aims to assess the influence of key physiological and clinical factors on the outcomes of heart failure patients at the Institute of Cardiology and Allied Hospital, Faisalabad, Pakistan, during April-December 2015. We will examine variables such as creatinine levels, gender, age, ejection fraction, blood pressure, anemia, and serum sodium to determine their impact on patient prognosis. Utilizing a range of analytic techniques including exploratory data analysis, nonparametric methods, Kaplan-Meier curves, Cox proportional hazards modeling, parametric survival models, and validation procedures, our study is designed to identify crucial predictors of heart failure and to investigate their influences on the outcome. The insights gained from this analysis are expected to contribute significantly to the development of tailored treatment strategies and improved risk stratification models, thereby enhancing clinical decision-making and patient care for heart failure management.
...
After checking the histograms, two continuous variables creatinine phosphokinase (cpk) and serum creatinine are right-skewed. We decide to log-transformed both of them for further model fitting. The new log-transformed variables follow an approximately symmetric distribution and are stored as `logcre` and `logcpk`.
...
model_selection |> kable(booktabs = T,
                         caption = "Table 4: Summary of Model Selection", digits = 4) |>
  kable_styling(latex_options = c("HOLD_position"), font_size = 10) 
```

The **Table 4** summarizes the Cox model selection process, indicating `logcre`, `age`, `EF` categories, and `BP` as persistent predictors across the top four models. The AIC favors `logcre` and `age` for their significant contributions to model fit relative to added complexity, emphasizing model fit over parameter count. 
...
model_summary |> kable(booktabs = T,
      caption = "Table 5: Summary of Cox Proportional Hazard Model", 
      digits = 4, row.names = TRUE) |>
  kable_styling(latex_options = c("HOLD_position"), font_size = 10) 
```

For the `logcre`, the hazard ratio of 3.0573 suggests that as the logarithm of creatinine levels increases by one unit, the hazard, or the risk of the event occurring, is more than three times greater. This result is statistically significant, indicating a strong relationship between creatinine levels and survival.

Concerning `age`, an HR of 1.0473 indicates that with each additional year of age, the hazard increases by approximately 4.73%. This relationship is highly significant, affirming age as a critical predictor in the survival model.

For `EF_catMedium`, and  `EF_catHigh` the HR suggests that individuals with a medium ejection fraction or high fraction ejection have lower risks of the event occurring compared to the baseline low ejection fraction, which are statistically significant.

The `bp` shows an HR of 1.7258, indicating that higher blood pressure status is associated with a 72.58% increase in hazard, and this predictor is statistically significant.

Lastly, `sodium` presents an HR of 0.9628, suggesting a slight decrease in hazard with higher sodium levels. However, this effect is not statistically significant, implies this predictor is not a conclusive predictor in this model.

Overall, this model provides a quantitative assessment of how each predictor influences the survival probability, with `logcre`, `age`, `EF_catMedium`, `EF_catHigh`, and `bp` being significant factors in predicting survival outcomes.
...
# Create table using kable
weibull_summary %>% kable(caption = "Table 6: Summary of Weibul PH Model Fitting", digits = 4) |>
  kable_styling(latex_options = c("HOLD_position"), font_size = 10) 
```

The Weibull model obtained from Stepwise Selection using AIC contains bp, age, EF_cat, sodium, and logcre.

For `bp`, it is a binary covariate  (0 and 1), the coefficient is 0.559, implying a 1.749 times higher hazard of the event in level 1 compared to level 0. The Wald p-value is 0.009 < 0.05, indicating statistical significance.

For `age`, for each one-unit increase in age, the hazard (risk) of the event increases by a factor of 1.049. The coefficient is statistically significant with p-value < 0.001.

For `EF_cat`, it is a categorical variable with three levels: Low, Medium, and High.
The medium Ejection Fraction has a coefficient of -1.145, indicating a 0.318 times lower hazard compared to the low Ejection Fraction. The Wald p-value < 0.001, which is statistically significant.
The high Ejection Fraction has a coefficient of -0.978, implying a 0.376 times lower hazard compared to the low Ejection Fraction. The Wald p-value < 0.001, which is statistically significant.

For `sodium`, for each unit increase in sodium level, the hazard decreases by a factor of 0.963. Although its p-value = 0.117 > 0.05, it was still considered small enough to be statistically significant.

For `logcre`, for each unit increase in logcre, the hazard increases by a factor of 3.034. The coefficient is statistically significant with p-value: < 0.001.

The `overall p-value` = 5.55112e-16 with 6 dergree of freedom, which is statistically significant. It sugguest that there is strong evidence against the null hypothesis that none of the predictors are associated with the heart failure.


```{r gompertz}
## gompertz
fit_gompertz <- eha::phreg(Surv(time, event) ~ gender + smoking + diabetes + bp + anemia + 
                  age + EF + sodium + platelets + log(creatinine+1) + log(cpk+1), 
                data = model_data, dist = "gompertz")
# AIC Both direction
# stepAIC(fit_gompertz, direction = "backward")

final_gompertz <- eha::phreg(formula = Surv(time, event) ~ bp + anemia + age + EF_catMedium + 
...
# Create table using kable
gompertz_summary %>% kable(caption = "Summary of Gompertz PH Model Fitting", digits = 4) |>
  kable_styling(latex_options = c("HOLD_position"), font_size = 10) 

```

The Gompertz model obtained from stepwise selection using AIC contains anemia, sodium, bp, log(cre), ef_cat and age. 

For `bp`, when comparing level 1 to the reference level 0, the coefficient of bp is 0.558, indicating a 1.747 times higher hazard of the event in level 1 compared to level 0. The Wald p-value is 0.009, indicating statistical significance.

For `anemia`, for the variable "anemia," comparing level 1 to the reference level 0, the coefficient of anemia is 0.303, suggesting a 1.354 times higher hazard of the event in individuals with anemia compared to those without. Although its p-value = 0.149 > 0.05, it was still considered small enough to be statistically significant.

For `age`, for each one-unit increase in age, the hazard (risk) of the event increases by a factor of 1.049. The coefficient is statistically significant with p-value < 0.001.

For `ef_cat`, Compared to the reference level Low, the Medium Ejection Fraction has a coefficient of -1.116, indicating a 0.328 times lower hazard compared to the Low Ejection Fraction. The effect is statistically significant with Wald p-value: < 0.001.
Compared to the reference level Low, The High Ejection Fraction has a coefficient of -1.001, implying a 0.368 times lower hazard compared to the Low Ejection Fraction. The effect is statistically significant with Wald p-value: < 0.001.

For `sodium`, for each unit increase in sodium level, the hazard decreases by a factor of 0.959. Although its Wald p-value: 0.08, it was still considered small enough to be statistically significant.

For `logcre`, for each unit increase in logcre, the hazard increases by a factor of 3.035. The coefficient is statistically significant with Wald p-value: < 0.001.

The `overall p-value`=1.11022e-16 with 7 degree of freedom, which is statistically significant. It sugguest that there is strong evidence against the null hypothesis that none of the predictors are associated with heart failure. Therefore, it indicates the statistical significance of the overall model, implying that at least one predictor in the model is significantly associated with heart failure.
...
```{r, fig.align='center', fig.height=5, fig.width=8}
step_model_final <- coxph(Surv(time, event == 1) ~ EF_cat + bp + creatinine_log + age + sodium, data = mc_dat)

# Calculate predicted risks
predicted_risks <- predict(step_model_final, newdata = mc_dat, type = "risk")
 # Time points for ROC analysis
time_points <- c(50, 250)
 
# Calculate ROC curves at specified times
roc_50 <- timeROC(T = mc_dat$time, delta = mc_dat$event, marker = predicted_risks, times = 50, cause = 1)
roc_250 <- timeROC(T = mc_dat$time, delta = mc_dat$event, marker = predicted_risks, times = 250, cause = 1)
...
boot_c_statistic <- function(original_data, indices) {
 ...
 fit <- coxph(Surv(time, event == 1) ~ EF_cat + bp + creatinine_log + age + sodium, data = mc_dat)
 ...
 boot_results_c_stat <- boot(data = mc_dat, statistic = boot_c_statistic, R = 400)
...
fit <- coxph(Surv(time, event == 1) ~ EF_cat + bp + creatinine_log + age + sodium, data = mc_dat)
 ...
 # Perform bootstrap
 set.seed(123)
 boot_results_logistic <- boot(data = mc_dat, statistic = boot_calibration_logistic, R = 400)
...
The Cox proportional hazard model's results indicating creatinine as a significant predictor of survival are reasonable and supported by existing literature. A study published in Frontiers in Cardiovascular Medicine found that postoperative serum creatinine is a significant prognostic factor for cardiac surgery patients (Zhong et al., 2021). This study showed that higher levels of postoperative serum creatinine were linked to increased hospital mortality and longer stays in the intensive care unit. Specifically, it found that patients who did not survive had significantly higher postoperative serum creatinine levels, and there was a positive correlation between these levels and lengths of ICU stay. These findings highlight the importance of renal function, which is intricately linked with heart failure, as indicated by creatinine levels, in determining the survival outcomes for patients undergoing cardiac surgery.
...
According to the Gompertz model and the Weibull model, there are three variables that have the smallest p-values in both models, namely age, ef_cat, and logcre. The results emphasize the importance of age, Ejection Fraction, and creatinine levels in predicting heart failure. A study published by Revista Española de Cardiología found that a n normal creatinine levels may mask pre-existing kidney failure, and kidney failure is an important prognostic factor in patients with heart failure. According to the results of the study, age, gender, cause of heart failure, left ventricular ejection fraction, diabetes mellitus, and hypertension were significant factors influencing mortality, while creatinine clearance remained an independent predictor of mortality (Zamora et al., 2007). Therefore, these results suggest the importance of considering age, Ejection Fraction, and creatinine levels in assessing the risk and prognosis of heart failure, possibly aiding in early identification, preventive measures, and targeted interventions in clinical practice.

# Conclusions

The non-parametric method showed that more than 50% of both genders (54% for males and 59% for females) can survive for over 300 days. This may indicate a high life expectancy and improved health care. Moreover, the non-parametric method showed no significant difference in survival probability between genders. This result was later consistent with the results of the model selection. Our Cox proportional hazard model

In conclusion, our comprehensive analysis of heart failure using the Cox proportional hazards model has revealed important insights into the factors influencing survival. While the model demonstrates strong discriminative ability, as evidenced by the C-index of around 0.737 and AUC values ranging from 0.736 at 50 days to 0.934 at 250 days, indicating improved discrimination over time, it also suggests the need for refinement in risk predictions. Specifically, the calibration slope of approximately 1.33 points to a mild overestimation in risk predictions, hinting at potential overfitting. This aspect is crucial in clinical contexts, where precise risk assessment is key to patient care. Our findings underscore the importance of not only assessing a model's ability to discriminate but also ensuring its predictions accurately reflect observed outcomes. Future work will focus on refining the model's complexity, exploring alternative approaches, and conducting external validation to enhance its accuracy and applicability in diverse clinical settings. This study, by identifying significant predictors such as creatinine levels, age, ejection fraction, and blood pressure, contributes to a deeper understanding of heart failure progression and lays the groundwork for more personalized and effective patient management strategies.
...
4. Steyerberg, E. W., & Vergouwe, Y. (2014). Towards better clinical prediction models: Seven steps for development and an ABCD for validation. European Heart Journal, 35(29), 1925–1931. https://doi.org/10.1093/eurheartj/ehu207
...
6. Heagerty, P. J., & Zheng, Y. (2005). Survival Model Predictive Accuracy and ROC Curves. Biometrics, 61(1), 92–105. https://doi.org/10.1111/j.0006-341X.2005.030814.x
